...
namespace Giant.Framework
{
    ...
    public class Handle_NotifyService : MHandler<Msg_Service_Info>
    {
        public override async Task Run(Session session, Msg_Service_Info message)
        {
            ...
            if (Scene.Pool.GetComponent<NetGraphDataList>().NeedConnect(Scene.AppConfig.AppType, Scene.AppConfig.AppId, appType, message.AppId))
            {
                AppConfig config = AppConfigDataList.Instance.GetNetConfig(appType, message.AppId, message.SubId);
                if (config == null)
                {
