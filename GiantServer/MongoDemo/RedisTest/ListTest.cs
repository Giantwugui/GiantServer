...
namespace Redis.HelperTest
{
    ...
    public class ListTest
    {
        ...
        public void TestListAction()
        {
            ...
            //左进
            UserModel leftUser = new UserModel() { Id = 1, UserName = "Giant", Age = 33 };
            Assert.AreEqual(1, redis.ListLeftPush<UserModel>(key, leftUser));
            List<UserModel> leftUserList = new List<UserModel>();
            for (int i = 2; i < 5; i++)
            {
                leftUserList.Add(new UserModel { Id = i, UserName = $"Giant{i}", Age = 33 + i });
            }
            ...
            UserModel rightUser = new UserModel() { Id = 21, UserName = "Giant", Age = 33 };
            ...
            for (int i = 5; i < 8; i++)
            {
                rightUserList.Add(new UserModel { Id = i, UserName = $"Giant{i}", Age = 33 + i });
            }
            ...
            //在指定值之前插入一条数据
            UserModel beforeUser = new UserModel() { Id = 20, UserName = "Giant_Before", Age = 33 };
            Assert.AreEqual(5, redis.ListInsertBefore(key, rightUser, beforeUser));


            //在指定值之后插入一条数据
            UserModel afterUser = new UserModel() { Id = 22, UserName = "Giant_After", Age = 33 };
            ...
            UserModel RightPopLeftPushUser = new UserModel() { Id = 20, UserName = "Giant_Before", Age = 33 };
